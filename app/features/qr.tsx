...
export function QrCard({ student, school }: {
    ...
    const [src, setSrc] = useState(''), [file, setFile] = useState<File | null>(null), [fallback, setFallback] = useState(false), [message, setMessage] = useState('');
    useEffect(() => { let alive = true; setSrc(''); setFile(null); setFallback(false); QRCode.toDataURL('madrasetna:v1:' + student.qr_token, { width: 500, margin: 4, errorCorrectionLevel: 'M' }).then(s => { if (!alive)
        return; const bytes = Uint8Array.from(atob(s.split(',')[1]), c => c.charCodeAt(0)); setSrc(s); setFile(new File([bytes], student.username + '-qr.png', { type: 'image/png' })); }).catch(() => setMessage('تعذر تجهيز الرمز. حاول إعادة فتح البروفايل.')); return () => { alive = false; }; }, [student.qr_token, student.username]);
    async function share() { if (!file)
        return; if (navigator.canShare?.({ files: [file] }) && navigator.share) {
        try {
            await navigator.share({ files: [file], title: school, text: 'كود حضور ' + student.full_name + ' — ' + school });
            return;
        }
        catch (e) {
    ...
    } setFallback(true); setMessage('نزّل صورة الكود، وافتح واتساب وأرفقها في المحادثة.'); }
    const whatsapp = 'https://wa.me/?text=' + encodeURIComponent('كود حضور ' + student.full_name + ' — ' + school + ' (أرفق صورة QR بعد تنزيلها)');
    return <div className="qr-card"><b>{school}</b>{src ? <img src={src} width="240" height="240" alt={'كود الحضور الخاص بـ ' + student.full_name}/> : <p>جاري تجهيز الرمز…</p>}<h3>{student.full_name}</h3><span dir="ltr">@{student.username}</span><p>اعرض الرمز للأدمن عند تسجيل الحضور.</p><div className="actions wrap">{src && <a className="secondary" href={src} download={student.username + '-qr.png'}><Download size={17}/>حفظ QR</a>}<button className="primary" disabled={!file} onClick={() => void share()}><Share2 size={17}/>مشاركة على واتساب</button></div><small>اختَر واتساب من قائمة مشاركة الموبايل.</small>{message && <p role="status">{message}</p>}{fallback && <a className="secondary" href={whatsapp} target="_blank" rel="noopener noreferrer">فتح واتساب وإرفاق الصورة</a>}</div>;
}
export function Scanner({ onScan, disabled }: {
    ...
    async function read(value: string) {
        if (locked.current || disabledRef.current || Date.now() - last.current < 2500)
            return;
        locked.current = true;
