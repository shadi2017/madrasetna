...
export function EvaluationForm({ record, config, busy, onSave }: {
    ...
    return <form onSubmit={async (e) => { e.preventDefault(); const next = (e.nativeEvent as SubmitEvent).submitter?.getAttribute('value') === 'next'; try {
        if(categories.some(([k])=>k!=='quiz' && scores[k]!==undefined && scores[k]!==0 && scores[k]!==10)){toast.error('اختَر 0 أو 10 بدل الدرجات السابقة، ما عدا الكويز.');return;}
        const row = await onSave(present, scores, notes, version, next);
        setVersion(row.version);
    }
    catch { } }}><div className="attendance-switch"><label htmlFor="present">حضور اليوم <small>{config.daily_max.attendance} درجة عند الحضور</small></label><Switch id="present" dir="ltr" checked={present} onCheckedChange={setPresent}/></div><div className="grade-inputs">{categories.map(([key, label]) => <label key={key}>{label}{key !== 'quiz' ? <select aria-label={label} value={scores[key] ?? 0} onChange={e=>setScores({...scores,[key]:Number(e.target.value)})}><option value={0}>0 — لم يحصل عليها</option><option value={10}>10 — الدرجة كاملة</option>{scores[key]!==undefined && scores[key]!==0 && scores[key]!==10 && <option disabled value={scores[key]}>درجة سابقة: {scores[key]} — اختر 0 أو 10</option>}</select> : <div className="score-input"><input type="number" min="0" max={config.daily_max[key]} step="any" value={scores[key] ?? ''} onChange={e => setScores({ ...scores, [key]: e.target.value === '' ? 0 : Number(e.target.value) })} placeholder="0"/><span>/ {config.daily_max[key]}</span></div>}</label>)}</div><details><summary>ملاحظة للطالب</summary><label>ملاحظات<textarea rows={2} maxLength={2000} value={notes} onChange={e => setNotes(e.target.value)}/></label></details><div className="actions wrap"><button disabled={busy} className="primary" value="save">حفظ تقييم اليوم</button><button disabled={busy} className="secondary" value="next">حفظ والتالي</button></div></form>;
}
export function SettingsForm({ settings, busy, onSave }: {
    ...
    return <form onSubmit={e => { e.preventDefault(); void onSave(draft); }}><label>اسم الدراسة<input required maxLength={100} value={draft.name} onChange={e => setDraft({ ...draft, name: e.target.value })}/></label><label>الشعار النصي<input maxLength={250} value={draft.slogan} onChange={e => setDraft({ ...draft, slogan: e.target.value })}/></label><label>العبارة الصغيرة في شاشة الدخول<input maxLength={250} value={draft.login_tagline ?? 'كل طالب له مكان'} onChange={e=>setDraft({...draft,login_tagline:e.target.value})}/></label><label>العنوان الرئيسي في شاشة الدخول<textarea maxLength={500} rows={2} value={draft.login_title ?? 'أيام بنعيشها.\nوخطوات بنكبرها.'} onChange={e=>setDraft({...draft,login_title:e.target.value})}/></label><label>الآية<textarea maxLength={1000} rows={3} value={draft.verse} onChange={e => setDraft({ ...draft, verse: e.target.value })}/></label><label>اللوجو<input type="file" accept="image/png,image/jpeg,image/webp" onChange={async (e) => {
