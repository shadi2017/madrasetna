...
export function ExcelPanel({ data, busy, demo, mutate, account }: {
    ...
    async function download(template = false) { setWorking(true); try {
    ...
    return <><div className="excel-grid"><section className="panel"><FileSpreadsheet className="feature-icon"/><h2>تصدير البيانات</h2><p className="muted">الطلاب والأيام والتقييمات والإعدادات والسجل في ملف واحد. كلمات المرور لا تدخل في التصدير.</p><div className="actions wrap section-gap"><button className="primary" disabled={busy || working} onClick={() => void download()}><Download size={18}/>تصدير Excel</button><button className="secondary" disabled={working} onClick={() => void download(true)}>تنزيل قالب فارغ</button></div><small>السجل ورموز QR للتصدير فقط. إعادة إنشاء حساب تُصدر رمزًا وكلمة مرور جديدين.</small></section><section className="panel"><Upload className="feature-icon"/><h2>استيراد ملف</h2><p className="muted">اختر ملف XLSX، راجع المعاينة، ثم أكّد الحفظ.</p><label className="upload-zone">ملف Excel (حتى 5 ميجابايت)<input type="file" accept=".xlsx" disabled={working || busy} onChange={async (e) => { const file = e.target.files?.[0]; if (!file)
        return; setWorking(true); setMessage(''); setReport([]); setDone(false); setItems([]); try {
