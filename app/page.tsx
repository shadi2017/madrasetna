...
import { PublicationControls } from './features/publication';
...
export default function App() {
    ...
    <main className="workspace">{loading ? <div className="empty">جاري تحميل البيانات…</div> : !admin && (!studentSelf || studentSelf.status !== 'active') ? <Empty title={studentSelf?.status === 'pending' ? 'طلبك بانتظار التفعيل' : studentSelf?.status === 'rejected' ? 'طلبك لم تتم الموافقة عليه' : 'الحساب غير نشط'}>{studentSelf?.status === 'pending' ? 'تم تسجيل بياناتك. الأدمن هيراجع الطلب ويفعّل حسابك؛ الصفحة هتتحدث تلقائيًا بعد الموافقة.' : 'تواصل مع أدمن الدراسة لمراجعة حسابك.'}</Empty> : <>
        ...
        {view === 'grading' && admin && <><PageHeading title="نظام الدرجات وإعلان النتيجة" subtitle="حدد النهايات الأصلية والتحويل، وعدد أجزاء الحفظ."/><div className="form-panel"><PublicationControls grading={data.grading} busy={disabled} onSave={async (field,value)=>{await mutate('set_publication',{p_field:field,p_value:value,p_version:data.grading.version});}}/><GradingForm key={data.grading.version} grading={data.grading} busy={disabled} onSave={async (config, version) => { try {
            await mutate('save_grading_config', { p_config: config, p_version: version });
            setView('home');
        }
        catch { } }}/></div></>}
        ...
        {view === 'settings' && admin && <><PageHeading title="إعدادات الدراسة" subtitle="الاسم واللوجو والشعار والآية، زي ما تحبهم."/><section className="panel form-panel"><SettingsForm key={view} settings={data.settings} busy={disabled} onSave={async (s) => { await mutate('save_settings_v2', { p_login_tagline: s.login_tagline ?? 'كل طالب له مكان', p_login_title: s.login_title ?? 'أيام بنعيشها.\nوخطوات بنكبرها.', p_name: s.name, p_slogan: s.slogan, p_verse: s.verse, p_logo: s.logo, p_version: s.version }).then(() => setView('home')).catch(() => { }); }}/></section></>}
    ...
    {owner && resetStudent && <Modal title={'كلمة مرور جديدة · ' + resetStudent.full_name} description="بعد الحفظ، سلّم كلمة المرور الجديدة للطالب بشكل خاص." onClose={() => !busy && setResetStudent(null)}><form onSubmit={async (e) => {
                e.preventDefault();
                try {
