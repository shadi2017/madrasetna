...
import { BookOpen, QrCode, ShieldCheck, Users, ArrowLeft, LayoutDashboard, CalendarDays, ClipboardList, History, Settings as SettingsIcon, LogOut, Plus, Search, ChevronLeft, Download, FileSpreadsheet, RotateCcw, Trash2, KeyRound, Pencil, RefreshCw, Check, WifiOff, GraduationCap } from 'lucide-react';
...
type View = 'home' | 'students' | 'scan' | 'grades' | 'days' | 'history' | 'settings' | 'excel' | 'profile';
const nav = [['home', 'نظرة عامة', LayoutDashboard], ['students', 'الطلاب', Users], ['scan', 'تسجيل الحضور', QrCode], ['grades', 'التقييمات', ClipboardList], ['days', 'أيام الدراسة', CalendarDays], ['excel', 'استيراد وتصدير', FileSpreadsheet], ['history', 'السجل والمحذوفات', History], ['settings', 'إعدادات الدراسة', SettingsIcon]] as const;
...
export default function App() {
    ...
    useEffect(() => { if (!supabase)
        return; const { data: listener } = supabase.auth.onAuthStateChange((_event, session) => { setUser(session?.user.id || null); setAuthReady(true); }); return () => listener.subscription.unsubscribe(); }, []);
    useEffect(() => { const on = () => setOnline(true), off = () => setOnline(false); window.addEventListener('online', on); window.addEventListener('offline', off); return () => { window.removeEventListener('online', on); window.removeEventListener('offline', off); }; }, []);
    const refresh = useCallback(async () => { if (demo || !user)
        return; const serial = ++request.current; try {
    ...
    useEffect(() => { if (!user || demo)
        return; setLoading(true); void refresh(); return () => { request.current++; }; }, [user, demo, refresh]);
    useEffect(() => { if (user || !supabase || demo)
        return; let alive = true; supabase.from('settings').select('*').eq('id', 1).single().then(({ data: s }) => { if (alive && s)
        setData(d => ({ ...d, settings: s })); }); return () => { alive = false; }; }, [user, demo]);
    useEffect(() => { if (!user || demo || !supabase)
        return; const client = supabase; let timer: ReturnType<typeof setTimeout>; const channel = client.channel('school-' + user); for (const table of ['profiles', 'days', 'evaluations', 'settings'])
        channel.on('postgres_changes', { event: '*', schema: 'public', table }, () => { clearTimeout(timer); timer = setTimeout(() => void refresh(), 250); }); channel.subscribe(status => setSync(status)); const fallback = setInterval(() => { if (navigator.onLine)
        void refresh(); }, 30000); const focus = () => void refresh(); window.addEventListener('focus', focus); window.addEventListener('online', focus); return () => { clearTimeout(timer); clearInterval(fallback); window.removeEventListener('focus', focus); window.removeEventListener('online', focus); void client.removeChannel(channel); }; }, [user, demo, refresh]);
    ...
    }).modelContext; if (!context || (!user && !demo))
        return; const lifecycle = new AbortController(); void Promise.resolve(context.registerTool({ name: 'show_student_search', title: 'البحث عن طالب', description: 'Open the visible student search. This does not edit records.', inputSchema: { type: 'object', properties: { query: { type: 'string', maxLength: 100 } }, required: ['query'], additionalProperties: false }, annotations: { readOnlyHint: false }, execute: (input: unknown) => { if (!admin)
            throw Error('ADMIN_REQUIRED'); const q = (input as {
            query?: unknown;
        })?.query; if (typeof q !== 'string' || q.length > 100)
            throw Error('INVALID_QUERY'); setQuery(q); setPage(0); setView('students'); return { opened: true, query: q }; } }, { signal: lifecycle.signal })).catch(() => { }); return () => lifecycle.abort(); }, [admin, user, demo]);
    async function mutate(name: string, args: Record<string, unknown>, silent = false) { if (working.current)
        throw Error('BUSY'); if (!demo && !navigator.onLine)
        throw Error('Failed to fetch'); working.current = true; setBusy(true); try {
        let result;
        if (demo) {
    ...
    async function manage(body: Record<string, unknown>) { if (working.current)
        throw Error('BUSY'); working.current = true; setBusy(true); try {
        let result;
        if (demo) {
    ...
    function startDemo(student = false) { const d = makeDemo(); update(d); setDemo(true); setDemoStudent(student); setView(student ? 'profile' : 'home'); setError(''); }
    async function logout() { request.current++; update(emptyData); setUser(null); setDemo(false); setDemoStudent(false); setView('home'); setProfile(null); setCredentials(null); if (supabase) {
    ...
    const studentSelf = demo ? students[0] : students.find(s => s.id === user);
    ...
    {admin && editDay && <DayForm day={editDay === 'new' ? undefined : editDay} busy={disabled} onClose={() => !busy && setEditDay(null)} onSave={async (label, date) => { await mutate('save_day', { p_id: editDay === 'new' ? null : editDay.id, p_label: label, p_date: date || null, p_version: editDay === 'new' ? 0 : editDay.version }).then(() => setEditDay(null)).catch(() => { }); }}/>}
    {admin && profile && <Modal title="بروفايل الطالب" onClose={() => setProfile(null)} wide description="بيانات الطالب وكود الحضور وتفاصيل التقييم."><StudentProfile student={data.students.find(s => s.id === profile.id) || profile} data={data}/><div className="actions wrap"><button className="secondary" disabled={disabled} onClick={() => { setEditStudent(data.students.find(s => s.id === profile.id) || profile); setProfile(null); }}><Pencil size={16}/>تعديل البيانات</button><button className="secondary" disabled={disabled} onClick={() => { setSelectedStudent(profile.id); setProfile(null); setView('grades'); }}>تقييم الطالب</button><button className="secondary" disabled={disabled} onClick={() => { setResetStudent(profile); setNewPassword(password()); setProfile(null); }}><KeyRound size={16}/>باسورد جديد</button><button className="secondary" disabled={disabled} onClick={() => setConfirm({ title: 'تغيير كود الحضور؟', description: 'الكود القديم هيتوقف عن العمل. الطالب هيستخدم الكود الجديد من بروفايله.', action: () => void mutate('rotate_qr', { p_id: profile.id }).catch(() => { }) })}>تجديد QR</button><button className="danger" disabled={disabled} onClick={() => { remove('profiles', data.students.find(s => s.id === profile.id) || profile, profile.full_name); setProfile(null); }}><Trash2 size={16}/>حذف</button></div></Modal>}
    {admin && resetStudent && <Modal title={'كلمة مرور جديدة · ' + resetStudent.full_name} description="بعد الحفظ، سلّم كلمة المرور الجديدة للطالب بشكل خاص." onClose={() => !busy && setResetStudent(null)}><form onSubmit={async (e) => { e.preventDefault(); try {
        ...
        toast.error(safeError(e));
    } }}><label>كلمة المرور الجديدة<input required minLength={10} maxLength={128} value={newPassword} dir="ltr" autoComplete="new-password" onChange={e => setNewPassword(e.target.value)}/></label><button className="primary" disabled={disabled}>حفظ كلمة المرور</button></form></Modal>}
...
function Login({ settings, onDemo }: {
    settings: Data['settings'];
    onDemo: (student?: boolean) => void;
}) { const [username, setUsername] = useState(''), [pass, setPass] = useState(''), [busy, setBusy] = useState(false), [error, setError] = useState(''); return <main className="login-page" dir="rtl"><section className="login-brand"><div className="brand">{settings.logo ? <img src={settings.logo} alt="لوجو الدراسة"/> : <BookOpen />}<b>{settings.name}</b></div><span className="eyebrow">{settings.slogan || 'كل طالب له مكان'}</span><h1>أيام بنعيشها.<br />وخطوات بنكبرها.</h1><p>{settings.verse || 'الطلاب، الحضور، والتقييمات. كل تفاصيل الدراسة في مكان واحد.'}</p><div className="brand-features"><span><QrCode />حضور برمز خاص</span><span><Users />متابعة كل طالب</span><span><ShieldCheck />صلاحيات واضحة</span></div><small>مساحة تجمع الدراسة، وتقرّب المتابعة.</small></section><section className="login-side"><div className="login-box"><span className="eyebrow">أهلًا بيك</span><h2>تسجيل الدخول</h2><p>ادخل بيانات حسابك لمتابعة الدراسة.</p><form onSubmit={async (e) => { e.preventDefault(); if (!supabase)
    return; setBusy(true); setError(''); const login = username.trim().toLowerCase(); try {
    ...
    setBusy(false);
} }}><label>اسم المستخدم أو بريد الأدمن<input required autoComplete="username" dir="ltr" value={username} onChange={e => setUsername(e.target.value)} placeholder="اسم المستخدم"/></label><label>كلمة المرور<input required type="password" autoComplete="current-password" value={pass} onChange={e => setPass(e.target.value)} placeholder="اكتب كلمة المرور"/></label>{error && <p role="alert" className="error">{error}</p>}<button className="primary" disabled={!configured || busy}>{busy ? 'جاري الدخول…' : 'تسجيل الدخول'}<ArrowLeft size={18}/></button></form><p className="login-help">نسيت كلمة المرور؟ تواصل مع أدمن الدراسة.</p>{!configured && <div className="notice"><b>نسخة جاهزة للتجربة</b><p>لم يتم ربط قاعدة البيانات بعد. جرّب الواجهة ببيانات توضيحية، بدون حفظ حقيقي.</p><div className="actions wrap"><button className="secondary" onClick={() => onDemo(false)}>تجربة الأدمن</button><button className="secondary" onClick={() => onDemo(true)}>تجربة الطالب</button></div></div>}</div></section></main>; }
...
}) { return <Table><TableHeader><TableRow><TableHead>الطالب</TableHead><TableHead>رقم الموبايل</TableHead><TableHead>الحضور</TableHead><TableHead>الدرجة</TableHead><TableHead><span className="sr-only">فتح البروفايل</span></TableHead></TableRow></TableHeader><TableBody>{students.map(s => { const sum = summary(s, data), present = data.evaluations.some(e => e.student_id === s.id && e.day_id === day?.id && e.present && !e.deleted_at); return <TableRow key={s.id}><TableCell><button className="student-name" onClick={() => onOpen(s)}><span className="avatar">{s.full_name[0]}</span><span><b>{s.full_name}</b><small dir="ltr">@{s.username}</small></span></button></TableCell><TableCell><span dir="ltr">{s.phone}</span></TableCell><TableCell><span className={'badge ' + (present ? 'success' : 'neutral')}>{!day ? 'بدون يوم' : present ? 'حاضر' : 'لم يسجل'}</span></TableCell><TableCell><b>{sum.points}</b><small className="muted"> / {sum.max}</small></TableCell><TableCell><button className="icon-button" aria-label={'فتح بروفايل ' + s.full_name} onClick={() => onOpen(s)}><ChevronLeft size={18}/></button></TableCell></TableRow>; })}</TableBody></Table>; }
function StudentProfile({ student, data }: {
    student: Student;
    data: Data;
}) { const sum = summary(student, data); return <div className="student-profile"><div className="profile-overview"><section className="profile-summary"><div className="avatar large">{student.full_name[0]}</div><h2>{student.full_name}</h2><span dir="ltr">{student.phone}</span><div className="profile-stats"><div><strong>{sum.present} / {sum.days}</strong><span>أيام الحضور</span></div><div><strong>{sum.points} / {sum.max}</strong><span>الدرجة النهائية</span></div></div><Progress value={sum.percent} aria-label="نسبة الدرجة النهائية"/><p className="muted">{sum.percent}% من مجموع أيام الدراسة</p><small>الحضور وكل بند من 10 يوميًا. غير المسجل يُحسب صفرًا.</small></section><QrCard student={student} school={data.settings.name}/></div><section className="panel no-pad"><div className="panel-heading"><h3>تفاصيل الحضور والتقييمات</h3></div><div className="table-scroll"><Table><TableHeader><TableRow><TableHead>اليوم</TableHead><TableHead>الحضور</TableHead>{categories.map(([k, l]) => <TableHead key={k}>{l}</TableHead>)}<TableHead>المجموع</TableHead></TableRow></TableHeader><TableBody>{data.days.filter(d => !d.deleted_at).sort((a, b) => (a.date || '9999').localeCompare(b.date || '9999') || a.label.localeCompare(b.label, 'ar', { numeric: true })).map(d => { const e = data.evaluations.find(e => e.student_id === student.id && e.day_id === d.id && !e.deleted_at); return <TableRow key={d.id}><TableCell><b>{d.label}</b><small className="block">{arDate(d.date)}</small></TableCell><TableCell><span className={'badge ' + (e?.present ? 'success' : 'neutral')}>{e?.present ? 'حاضر' : 'لم يسجل'}</span></TableCell>{categories.map(([k]) => <TableCell key={k}>{e?.scores[k] ?? '—'}</TableCell>)}<TableCell>{total(e)} / 110</TableCell></TableRow>; })}</TableBody></Table></div></section>{data.evaluations.filter(e => e.student_id === student.id && e.notes && !e.deleted_at && data.days.some(d => d.id === e.day_id && !d.deleted_at)).map(e => <div key={e.id} className="notice"><b>{data.days.find(d => d.id === e.day_id)?.label}</b><p>{e.notes}</p></div>)}</div>; }
function entityLabel(entity: string) { return ({ profiles: 'طالب', days: 'يوم دراسة', evaluations: 'تقييم', settings: 'إعدادات الدراسة' } as Record<string, string>)[entity] || entity; }
