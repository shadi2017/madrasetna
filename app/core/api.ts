...
export async function loadData(): Promise<Data> {
    ...
    if (error)
        throw error;
    const [students, days, evaluations, settings, audit] = await Promise.all([rows('profiles'), rows('days'), rows('evaluations'), rows('settings'), admin ? supabase!.from('audit_log').select('*').order('created_at', { ascending: false }).limit(200).then(({ data, error }) => { if (error)
            throw error; return data; }) : Promise.resolve([])]);
    return { ...emptyData, students, days, evaluations, settings: settings[0] || emptyData.settings, audit, admin: !!admin } as Data;
}
